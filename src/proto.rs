...
impl BitwiseImm {
    pub const ONE: BitwiseImm = BitwiseImm {
        immr: 0,
        imms: 0,
        n: 1,
    };
...
pub enum Instruction {
    LogicalRegister(InstructionLogicalShiftedRegister),
    AddSubRegister(InstructionLogicalShiftedRegister),
    AddSubImm(InstructionAddSubImm),
    ConditionalBranch(InstructionConditionalBranch),
    LogicalImm(InstructionLogicalImm),
    BitfieldImm(InstructionLogicalImm),
    MovWideImm(InstructionWideImm),
    DataProcessing1Src(InstructionDataProcessing),
    DataProcessing2Src(InstructionDataProcessing),
    DataProcessing3Src(InstructionDataProcessing),
    BranchImm26(InstructionImm26),
    BLR(InstructionBLR),
    Load(InstructionLoadStore),
    Store(InstructionLoadStore),
    RET,
}

#[derive(Copy, Clone)]
pub struct InstructionLogicalShiftedRegister {
...
impl Default for InstructionLogicalShiftedRegister {
    fn default() -> Self {
        Self {
...
#[derive(Copy, Clone)]
pub struct InstructionDataProcessing {
    pub size: OperandSize,
...
impl Default for InstructionDataProcessing {
    fn default() -> Self {
        Self {
            size: OperandSize::S64,
            opcode: 0,
...
#[derive(Copy, Clone)]
pub struct InstructionAddSubImm {
    pub size: OperandSize,
    pub opcode: u8,     // 1 bit
...
impl Default for InstructionAddSubImm {
    fn default() -> Self {
        Self {
            size: OperandSize::S64,
            opcode: 0,
...
#[derive(Copy, Clone)]
pub struct InstructionLogicalImm {
    pub size: OperandSize,
    pub opcode: u8, // 2 bits
    pub n: u8,      // negation (1 bit)
    pub dest: u8,   // Rd, 5 bits
...
impl Default for InstructionLogicalImm {
    fn default() -> Self {
        Self {
            size: OperandSize::S64,
            opcode: 0,
...
impl Default for InstructionWideImm {
    fn default() -> Self {
        Self {
            size: OperandSize::S64,
            opcode: 0,
...
#[derive(Copy, Clone)]
pub struct InstructionLoadStore {
    pub size: OperandSize,
...
impl Default for InstructionLoadStore {
    fn default() -> Self {
        Self {
            size: OperandSize::S64,
            data: 0,
            base: 0,
            mem: MemoryOperand::Offset(0), // default to an LDUR (no autoincrement)
        }
    }
}
...
impl Instruction {
    pub fn emit<C: ContextObject>(&self, jit: &mut JitCompiler<C>) {
        ...
        match self {
            Instruction::LogicalRegister(s) | Instruction::AddSubRegister(s) => {
                ...
                match self {
                    Instruction::LogicalRegister(_) => ins |= 0b01010u32 << 24,
                    Instruction::AddSubRegister(_) => ins |= 0b01011u32 << 24,
                    _ => unreachable!(),
                };
            ...
            Instruction::DataProcessing2Src(s) | Instruction::DataProcessing1Src(s) => {
                ins |= (s.dest & 0b11111) as u32;
                ins |= ((s.src1 & 0b11111) as u32) << 5;
                ins |= ((s.src2 & 0b11111) as u32) << 16;
                ins |= ((s.opcode & 0b111111) as u32) << 10;
                if let Instruction::DataProcessing1Src(_) = self {
                    ins |= 0b1u32 << 30
                }
            ...
            Instruction::DataProcessing3Src(s) => {
                ins |= (s.dest & 0b11111) as u32;
                ins |= ((s.src1 & 0b11111) as u32) << 5;
                ins |= ((s.src2 & 0b11111) as u32) << 16;
            ...
            Instruction::AddSubImm(s) => {
                ...
                match self {
                    Instruction::AddSubImm(_) => ins |= 0b10001u32 << 24,
                    _ => unreachable!(),
                };
            ...
            Instruction::LogicalImm(s) | Instruction::BitfieldImm(s) => {
                ins |= (s.dest & 0b11111) as u32;
                ins |= ((s.src & 0b11111) as u32) << 5;
                ...
                match self {
                    Instruction::LogicalImm(_) => ins |= 0b100100u32 << 23,
                    Instruction::BitfieldImm(_) => ins |= 0b100110u32 << 23,
                    _ => unreachable!(),
                };
            ...
            Instruction::MovWideImm(s) => {
                ...
                match self {
                    Instruction::MovWideImm(_) => ins |= 0b100101u32 << 23,
                    _ => unreachable!(),
                };
            ...
            Instruction::Load(s) | Instruction::Store(s) => {
                ins |= (s.data & 0b11111) as u32;
                ins |= ((s.base & 0b11111) as u32) << 5;
                let mode = match s.mem {
                    MemoryOperand::OffsetPreIndex(_) => 0b11,
                    MemoryOperand::OffsetPostIndex(_) => 0b01,
                    MemoryOperand::OffsetScaled(_) => 0b00, // spot used for imm12,
                    MemoryOperand::OffsetIndexShift(_, _) => 0b10,
                    MemoryOperand::Offset(_) => 0b00,
                };
                ins |= (mode as u32) << 10;

                // Encode the memory operand
                match s.mem {
                    MemoryOperand::OffsetPreIndex(imm9)
                    | MemoryOperand::OffsetPostIndex(imm9)
                    | MemoryOperand::Offset(imm9) => {
                        ins |= ((imm9 & 0b111111111) as u32) << 12;
                    }
                    MemoryOperand::OffsetScaled(imm12) => {
                        ins |= ((imm12 & 0b111111111111) as u32) << 10;
                    }
                    MemoryOperand::OffsetIndexShift(idx_reg, should_shift) => {
                        if should_shift {
                            ins |= 0b1u32 << 12;
                        }
                        ins |= 0b011u32 << 13;
                ...
                match s.mem {
                    MemoryOperand::OffsetPreIndex(_)
                    | MemoryOperand::OffsetPostIndex(_)
                    | MemoryOperand::Offset(_) => {
                        ins |= (if matches!(self, Instruction::Load(_)) {
                            0b111000010u32
                        } else {
                            0b111000000
                        }) << 21;
                    }
                    MemoryOperand::OffsetScaled(_) => {
                        ins |= (if matches!(self, Instruction::Load(_)) {
                            0b11100101u32
                        } else {
                            0b11100100
                    ...
                    MemoryOperand::OffsetIndexShift(_, _) => {
                        ins |= (if matches!(self, Instruction::Load(_)) {
                            0b111000011u32
                        } else {
                            0b111000001
    ...
    pub fn mov(size: OperandSize, source: u8, destination: u8) -> Self {
        ...
        Self::LogicalRegister(InstructionLogicalShiftedRegister {
            size,
            opcode: 1,
            n: 0,
            dest: destination,
            src1: SP_XZR,
            src2: source,
            ..InstructionLogicalShiftedRegister::default()
        })
    }
    ...
    #[must_use]
    pub fn tst_imm(source: u8, imm: BitwiseImm) -> Self {
        Self::LogicalImm(InstructionLogicalImm {
            size: OperandSize::S64,
            opcode: 3,
            n: imm.n,
            immr: imm.immr,
    ...
    pub fn add(size: OperandSize, src1: u8, src2: u8, destination: u8) -> Self {
        Self::AddSubRegister(InstructionLogicalShiftedRegister {
            size,
            opcode: 0,
            n: 0,
            dest: destination,
    ...
    pub fn add_imm(size: OperandSize, src: u8, imm12: u16, destination: u8) -> Self {
        ...
        Self::AddSubImm(InstructionAddSubImm {
            size,
            opcode: 0,
            dest: destination,
            src,
            imm12,
            ..InstructionAddSubImm::default()
        })
    }
    ...
    pub fn zero_extend_to_u64(from_size: OperandSize, source: u8, destination: u8) -> Self {
        match from_size {
            // UXTB
            OperandSize::S8 => Self::BitfieldImm(InstructionLogicalImm {
                ...
                dest: destination,
                src: source,
                ..InstructionLogicalImm::default()
            }),
            // UXTH
            OperandSize::S16 => Self::BitfieldImm(InstructionLogicalImm {
                size: OperandSize::S32,
                opcode: 2,
                immr: 0,
                imms: 15,
    ...
    pub fn sign_extend_to_i64(from_size: OperandSize, source: u8, destination: u8) -> Self {
        match from_size {
            ...
            // SXTW
            OperandSize::S32 => Self::BitfieldImm(InstructionLogicalImm {
                size: OperandSize::S64,
                n: 1,
                opcode: 0,
                immr: 0,
    ...
    pub fn lsl_imm(source: u8, shift_imm: u8, destination: u8) -> Self {
        ...
        Self::BitfieldImm(InstructionLogicalImm {
            size: OperandSize::S64,
            n: 1,
            opcode: 2,
    ...
    pub fn lsl_reg(size: OperandSize, src: u8, shift: u8, destination: u8) -> Self {
        Self::DataProcessing2Src(InstructionDataProcessing {
            size,
            opcode: 0b001000,
            dest: destination,
            src1: src,
            src2: shift,
            ..InstructionDataProcessing::default()
        })
    }
    ...
    pub fn rev(size: OperandSize, src: u8, destination: u8) -> Self {
        Self::DataProcessing1Src(InstructionDataProcessing {
            size,
            opcode: match size {
                OperandSize::S16 => 0b1,
                OperandSize::S32 => 0b10,
    ...
    pub fn b_cond(cond: Condition, imm19: i32) -> Self {
        Self::ConditionalBranch(InstructionConditionalBranch {
            cond: cond as u8,
            imm19,
        })
    }
    ...
    pub fn bl(imm26: i32) -> Self {
        Self::BranchImm26(InstructionImm26 {
            opcode: 0b100101,
            imm26,
        })
    }
    ...
    pub fn blr(target: u8) -> Self {
        Self::BLR(InstructionBLR { target })
    }
    ...
    pub fn movk(destination: u8, shift_16: u8, immediate: u16) -> Self {
        ...
        Self::MovWideImm(InstructionWideImm {
            size: OperandSize::S64,
            dest: destination,
            hw: shift_16,
            imm16: immediate,
    ...
    #[must_use]
    pub fn load(size: OperandSize, source: u8, indirect: MemoryOperand, data: u8) -> Self {
        exclude_operand_sizes!(size, OperandSize::S0);
        match indirect {
            MemoryOperand::OffsetPreIndex(_) | MemoryOperand::OffsetPostIndex(_) => {
                // in , loads with writeback to the base register cannot also use this
                // register as the dest
                debug_assert_ne!(source, data);
            }
            _ => {}
        }
        Self::Load(InstructionLoadStore {
            size,
            data,
            base: source,
            mem: indirect,
    ...
    #[must_use]
    pub fn store(size: OperandSize, data: u8, source: u8, indirect: MemoryOperand) -> Self {
        exclude_operand_sizes!(size, OperandSize::S0);
        match indirect {
            MemoryOperand::OffsetPreIndex(_) | MemoryOperand::OffsetPostIndex(_) => {
                // in , loads with writeback to the base register cannot also use this
        ...
        Self::Store(InstructionLoadStore {
            size,
            data,
            base: source,
            mem: indirect,
    ...
    pub fn push64(reg: u8) -> Self {
        ...
        Self::Store(InstructionLoadStore {
            ...
            mem: MemoryOperand::OffsetPreIndex(-16),
        })
    }
    ...
    pub fn pop64(reg: u8) -> Self {
        ...
        Self::Load(InstructionLoadStore {
            ...
            data: reg,
            base: SP_XZR, // this is SP in this context
            mem: MemoryOperand::OffsetPostIndex(16),
        })
    }
    ...
    pub fn madd(size: OperandSize, src1: u8, src2: u8, src3: u8, destination: u8) -> Self {
        Self::DataProcessing3Src(InstructionDataProcessing {
            size,
            opcode: 0b000,
            dest: destination,
            src1,
