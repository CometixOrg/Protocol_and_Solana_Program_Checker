...
use solana_rbpf::{
    ...
    program::{BuiltinFunction, BuiltinProgram, FunctionRegistry, Version},
...
macro_rules! test_syscall_asm {
    ...
    ($source:tt, $mem:tt, ($($syscall_number:literal => $syscall_name:expr => $syscall_function:expr),*$(,)?), $context_object:expr, $expected_result:expr $(,)?) => {
        ...
        for _version in [Version::V0, Version::V3] {
            config.enabled__versions = _version..=_version;
            let src = if _version == Version::V0 {
...
fn test_memory_instructions() {
    for _version in [Version::V0, Version::V3] {
        let config = Config {
            enabled__versions: _version..=_version,
            ..Config::default()
        };
...
fn test_err_dynamic_stack_out_of_bound() {
    let config = Config {
        enabled__versions: Version::V0..=Version::V3,
        max_call_depth: 3,
        ..Config::default()
    };
...
fn test_entrypoint_exit() {
    ...
    // frames.
    for highest__version in [Version::V0, Version::V3] {
        let config = Config {
            enabled__versions: Version::V0..=highest__version,
            ..Config::default()
        };
...
fn test_relative_call() {
    let config = Config {
        enabled__versions: Version::V0..=Version::V0,
        ..Config::default()
    };
    test_interpreter_and_jit_elf!(
        "tests/elfs/relative_call_v0.so",
        config,
        [1],
        (),
        TestContextObject::new(16),
...
fn test_err_callx_unregistered() {
    ...
    let config = Config {
        enabled__versions: Version::V3..=Version::V3,
        ..Config::default()
    };
    ...
    let versions = [Version::V0, Version::V3];
    let expected_errors = [
    ...
    for (version, error) in versions.iter().zip(expected_errors) {
        let config = Config {
            enabled__versions: *version..=*version,
            ..Config::default()
        };
    ...
    fn rust(
        ...
        if depth > 0 {
            ...
            let syscall_name = if version == 1 {
                config.enabled__versions = Version::V0..=Version::V0;
                "nested_vm_syscall"
            } else {
                config.enabled__versions = Version::V3..=Version::V3;
                "1"
            };
...
fn test_nested_vm_syscall() {
    ...
    let mut memory_mapping = MemoryMapping::new(vec![], &config, Version::V3).unwrap();
...
fn test_syscall_reloc_64_32() {
    ...
    test_interpreter_and_jit_elf!(
        "tests/elfs/syscall_reloc_64_32_v0.so",
        config,
        [],
        (
...
fn test_reloc_64_relative_v0() {
    ...
    test_interpreter_and_jit_elf!(
        "tests/elfs/reloc_64_relative_v0.so",
        config,
        [],
        (),
        TestContextObject::new(2),
...
fn test_reloc_64_relative_data_sbfv1() {
    ...
    test_interpreter_and_jit_elf!(
        "tests/elfs/reloc_64_relative_data_v0.so",
        config,
        [],
        (),
        TestContextObject::new(3),
...
#[test]
fn test_reloc_64_relative_data_v0() {
    // This test checks that the old behaviour is maintained for backwards
    // compatibility when dealing with non-v3 files. See also Elf::relocate().
    //
...
fn test_load_elf_rodata_v0() {
    ...
    test_interpreter_and_jit_elf!(
        "tests/elfs/rodata_section_v0.so",
        config,
        [],
        (),
        TestContextObject::new(3),
...
fn test_struct_func_pointer() {
    ...
    test_interpreter_and_jit_elf!(
        "tests/elfs/struct_func_pointer_v0.so",
        config,
        [],
        (),
        TestContextObject::new(2),
...
fn test_invalid_call_imm() {
    // In v3, `call_imm` N shall not be dispatched a syscall.
    let prog = &[
...
fn test_invalid_exit_or_return() {
    for _version in [Version::V0, Version::V3] {
        let inst = if _version == Version::V0 {
            0x9d
        } else {
            0x95
        };
