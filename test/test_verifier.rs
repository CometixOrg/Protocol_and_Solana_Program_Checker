...
fn test_verifier_err_invalid_reg_dst() {
    // r11 is disabled when _version.dynamic_stack_frames()=false, and only sub and add are
    // allowed when _version.dynamic_stack_frames()=true
    for highest__version in [Version::V0, Version::V3] {
        let executable = assemble::<TestContextObject>(
            "
            mov r11, 1
...
fn test_sdiv_disabled() {
    ...
    for (opc, instruction) in instructions {
        for highest__version in [Version::V0, Version::V3] {
            ...
            .unwrap();
            let result = executable.verify::<RequisiteVerifier>();
            if highest__version == Version::V3 {
                assert!(result.is_ok());
            } else {
